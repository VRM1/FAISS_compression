...
Supports both IVF and PQ compression methods with parallel processing for large datasets.
"""
...
def detect_compression_method(codebook_path, assignments_path):
    ...
    # Extract directory names and filenames from both paths
    paths_to_check = [codebook_path, assignments_path]
    
    for path in paths_to_check:
        # Split path into components and check each part
        path_parts = path.replace('\\', '/').split('/')
        
        for part in path_parts:
            # Check if any path component ends with _PQ or _IVF (case insensitive)
            if re.search(r'_PQ$', part, re.IGNORECASE):
                return 'PQ'
            elif re.search(r'_IVF$', part, re.IGNORECASE):
                return 'IVF'
            
            # Also check for patterns like clustering_something_PQ or clustering_something_IVF
            if re.search(r'clustering.*_PQ$', part, re.IGNORECASE):
                return 'PQ'
            elif re.search(r'clustering.*_IVF$', part, re.IGNORECASE):
                return 'IVF'
    
    # If no clear pattern found, try to infer from filename extensions or content
    for path in paths_to_check:
        filename = os.path.basename(path).lower()
        
        # Check if filename itself contains indicators
        if 'pq' in filename:
            return 'PQ'
        elif 'ivf' in filename:
            return 'IVF'
    
    # Last resort: provide helpful error message
    raise ValueError(
        f"Cannot auto-detect compression method from paths:\n"
        f"  Codebook: {codebook_path}\n"
        f"  Assignments: {assignments_path}\n"
        f"Ensure paths contain directories ending with '_PQ' or '_IVF', "
        f"or use --method to specify explicitly."
    )
...
def calculate_chunk_errors(reconstructed_chunk, true_vectors, chunk_start_idx, config):
    """Calculate errors for a random sample from the current chunk."""
    error_config = config.get('error_calculation', {})
    samples_per_chunk = error_config.get('samples_per_batch', 100)
    
    chunk_size = len(reconstructed_chunk)
    n_samples = min(samples_per_chunk, chunk_size)
    
    # Randomly sample indices from this chunk
    sample_indices = np.random.choice(chunk_size, n_samples, replace=False)
    
    errors = []
    for idx in sample_indices:
        true_idx = chunk_start_idx + idx
        if true_idx < len(true_vectors):
            reconstructed_vec = reconstructed_chunk[idx]
            true_vec = true_vectors[true_idx]
...
def write_chunk_to_csv(chunk_data, chunk_idx, temp_dir, headers, include_mrch_id, original_df, 
                      error_enabled, true_vectors, config, method):
    """Write a single chunk of data to a temporary CSV file."""
    vectors, start_idx, end_idx = chunk_data
    
    chunk_size = end_idx - start_idx
    batch_ids = list(range(start_idx, end_idx))
    
    # Calculate errors for this chunk if enabled
    all_errors = []
    if error_enabled and true_vectors is not None:
        chunk_errors = calculate_chunk_errors(vectors, true_vectors, start_idx, config)
        all_errors.extend(chunk_errors)
    
    # Create DataFrame for this chunk
    chunk_data_dict = {'row_id': batch_ids}
        ...
        try:
            ...
            for idx in range(start_idx, end_idx):
                if idx < len(original_df):
                    chunk_mrch_ids.append(original_df.iloc[idx]['mrch_id'])
                else:
                    chunk_mrch_ids.append(None)
            chunk_data_dict['mrch_id'] = chunk_mrch_ids
        except Exception as e:
            print(f"Warning: Error adding mrch_id for chunk {chunk_idx}: {e}")
    
    # Add embedding columns
    n_dimensions = vectors.shape[1]
    for i in range(n_dimensions):
        chunk_data_dict[f'emb{i+1}'] = vectors[:, i]
    
    chunk_df = pd.DataFrame(chunk_data_dict)
    
    # Write chunk to temporary file
    temp_file = os.path.join(temp_dir, f'chunk_{chunk_idx:06d}.csv')
    chunk_df.to_csv(temp_file, index=False, header=True)
    
    return temp_file, all_errors, chunk_size


def concatenate_csv_files(temp_files, output_path, headers):
    """Concatenate multiple CSV files into a single output file."""
    print("Concatenating chunk files into final CSV...")
    ...
    with open(output_path, 'w', newline='') as outfile:
        ...
        for temp_file in tqdm(temp_files, desc="Concatenating chunks"):
            with open(temp_file, 'r') as infile:
                # Skip header line of each chunk
                next(infile)
                # Copy all data lines
                shutil.copyfileobj(infile, outfile)


def write_reconstructed_vectors_to_csv_parallel(vectors, output_path, config, method, n_processes=None):
    """Write reconstructed vectors to CSV file using multiprocessing."""
    n_vectors, n_dimensions = vectors.shape
    
    # Determine number of processes
    if n_processes is None:
        n_processes = min(mp.cpu_count(), 8)  # Limit to 8 processes to avoid too many files
    ...
    # Load original dataframe for mrch_id
    original_df = None
    ...
    if include_mrch_id:
        # Insert mrch_id after row_id
        headers.insert(1, 'mrch_id')
    ...
    try:
        ...
        chunk_size = max(1, n_vectors // n_processes)
        chunks = []
        
        for i in range(n_processes):
            start_idx = i * chunk_size
            if i == n_processes - 1:  # Last chunk gets remainder
                end_idx = n_vectors
            else:
                end_idx = min((i + 1) * chunk_size, n_vectors)
            
            if start_idx < n_vectors:
                chunk_vectors = vectors[start_idx:end_idx]
                chunks.append((chunk_vectors, start_idx, end_idx))
        ...
        write_chunk_partial = partial(
            ...
            temp_dir=temp_dir,
            headers=headers,
            include_mrch_id=include_mrch_id,
        ...
        with Pool(processes=n_processes) as pool:
            # Use starmap to pass chunk_idx as second argument
            chunk_args = [(chunk, idx) for idx, chunk in enumerate(chunks)]
            results = list(tqdm(
                pool.starmap(write_chunk_partial, chunk_args),
                total=len(chunks),
        ...
        for temp_file, errors, chunk_size in results:
        ...
        # Concatenate all chunk files
        concatenate_csv_files(temp_files, output_path, headers)
        
        print(f"Successfully wrote {total_processed} reconstructed vectors to {output_path}")
        if include_mrch_id:
            print("✓ Included 'mrch_id' column in output CSV")
        
        # Report error statistics if collected
        error_stats = None
        if all_errors:
...
def write_reconstructed_vectors_to_csv_optimized(vectors, output_path, config, method):
    """Optimized sequential CSV writing using pandas bulk operations."""
    n_vectors, n_dimensions = vectors.shape
    
    print(f"Writing {n_vectors} reconstructed vectors with {n_dimensions} dimensions to {output_path}")
    
    # Load original dataframe for mrch_id if needed
    original_df = None
    include_mrch_id = False
    ...
    try:
        ...
        if data_path and os.path.exists(data_path):
            ...
            if original_df is not None and 'mrch_id' in original_df.columns:
                include_mrch_id = True
                print("✓ Found 'mrch_id' column - will include in output CSV")
    except Exception as e:
        print(f"Warning: Could not load original dataframe for mrch_id: {e}")
    
    # Create the complete dataframe at once
    print("Creating DataFrame...")
    data_dict = {'row_id': list(range(n_vectors))}
    
    # Add mrch_id if available
    if include_mrch_id and original_df is not None:
        try:
            mrch_ids = original_df['mrch_id'].iloc[:n_vectors].tolist()
            if len(mrch_ids) < n_vectors:
                mrch_ids.extend([None] * (n_vectors - len(mrch_ids)))
            data_dict['mrch_id'] = mrch_ids
        except Exception as e:
            print(f"Warning: Error adding mrch_id: {e}")
    
    # Add embedding columns efficiently
    print("Adding embedding columns...")
    for i in tqdm(range(n_dimensions), desc="Adding embedding columns"):
        data_dict[f'emb{i+1}'] = vectors[:, i]
    
    # Create DataFrame and write to CSV
    print("Creating final DataFrame and writing to CSV...")
...
def write_reconstructed_vectors_to_csv_fast(vectors, output_path, config, method, use_parallel=True, n_processes=None):
    """
    Fast CSV writing with option to use parallel processing or optimized sequential writing.
    
    Args:
        vectors: numpy array of reconstructed vectors
        output_path: path to output CSV file
        config: configuration dictionary
        method: compression method ('IVF' or 'PQ')
        use_parallel: whether to use parallel processing (default: True)
        n_processes: number of processes for parallel mode (default: auto)
    """
    
    # For smaller datasets, parallel processing might not be worth the overhead
    n_vectors = vectors.shape[0]
    parallel_threshold = config.get('output', {}).get('parallel_threshold', 100000)
    
    if use_parallel and n_vectors > parallel_threshold:
        return write_reconstructed_vectors_to_csv_parallel(vectors, output_path, config, method, n_processes)
    else:
        # Fall back to optimized sequential writing for smaller datasets
        print(f"Using sequential writing for {n_vectors} vectors (below parallel threshold of {parallel_threshold})")
        return write_reconstructed_vectors_to_csv_optimized(vectors, output_path, config, method)


def write_reconstructed_vectors_to_csv(vectors, output_path, config, method, batch_size=10000):
    """Original batch-based CSV writing function (kept for backward compatibility)."""
    n_vectors, n_dimensions = vectors.shape
...
def main():
    ...
    # New arguments for parallel processing
    parser.add_argument("--parallel", action="store_true", default=True, 
                       help="Use parallel processing for CSV writing (default: True)")
    parser.add_argument("--no-parallel", dest='parallel', action="store_false",
                       help="Disable parallel processing")
    parser.add_argument("--processes", type=int, default=None,
                       help="Number of processes for parallel writing (default: auto)")
    parser.add_argument("--legacy", action="store_true",
                       help="Use legacy batch-based writing method")
    ...
    use_parallel = args.parallel and config['output'].get('parallel_writing', True)
    n_processes = args.processes or config['output'].get('n_processes', None)
    ...
    if args.legacy:
        print("Using legacy batch-based CSV writing")
        batch_size = config['output'].get('batch_size', 10000)
        output_path, error_stats = write_reconstructed_vectors_to_csv(
            reconstructed_vectors, output_path, config, method, batch_size
        )
    else:
        if use_parallel:
            print("Using parallel CSV writing for improved performance")
        else:
            print("Using sequential CSV writing")
            
        output_path, error_stats = write_reconstructed_vectors_to_csv_fast(
